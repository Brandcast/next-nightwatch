module.exports = {
  '@tags' : ['filterspanel', 'contrast'],
    ...
    'Step 5: enter incrementally inscrease/decrease contrast via up/down arrow' : function (browser) { 
        ...
        .keys(browser.Keys.ARROW_UP)
        .verify.valueContains('.ui-wrapper[title="Contrast"] input[type=text]', '27')
        .keys(browser.Keys.RETURN)

        browser.frame(0) //selects iframe - must call to select anything within iframe
         .assert.cssProperty('.focused.component-wrapper.bc-text-wrapper', '-webkit-filter', 'contrast(0.27)')
        browser.frame(null)

        .keys(browser.Keys.ARROW_DOWN)
        .verify.valueContains('.ui-wrapper[title="Contrast"] input[type=text]', '26')
        .keys(browser.Keys.RETURN)
        ...
        browser.frame(0) //selects iframe - must call to select anything within iframe
         .assert.cssProperty('.focused.component-wrapper.bc-text-wrapper', '-webkit-filter', 'contrast(0.26)')
        browser.frame(null);
      browser.end();
    }
};